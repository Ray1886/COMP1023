...
def init_gameboard():
    ...
    b = []
    for i in range(20):
        b.append([int(0) for j in range(6)] )

    return b # this line also need to change
    ### TASK 1 ENDS HERE ###
...
def default_random_number_generator():
    ...
    index_1 = random.randint(0, len(candidates) - 1)  # Generate random number from 0 to len(candidates) - 1      
    index_2 = random.randint(0, len(candidates) - 1)
    return min(candidates[index_1],candidates[index_2])
    ### TASK 2 ENDS HERE ###
...
def is_valid_move(game_board, current_shape, current_location, current_rotation):
    ...
    ### TASK 3 STARTS HERE ###
    for i in range(4):
        block_row = (shapes[current_shape][current_rotation][i][0] + current_location[0])
        block_column = (shapes[current_shape][current_rotation][i][1] + current_location[1])
        if not((block_row in range(20)) and ((block_column in range(6))) and (game_board[block_row][block_column] == 0)):
            return False
    return True
...
def move_left(game_board, current_shape, current_location, current_rotation):
    ...
    new_location[1] = current_location[1] -1
    if is_valid_move(game_board, current_shape, new_location, current_rotation) == True:
        return new_location, current_rotation
    else:
...
def move_down(game_board, current_shape, current_location, current_rotation):
    ...
    ### TASK 6 STARTS HERE ###
    new_location = [6,6]
    new_location[0] = current_location[0] +1
...
def rotate(game_board, current_shape, current_location, current_rotation):
    ...
    if is_valid_move(game_board, current_shape, current_location, new_rotation) == True:
        return current_location, new_rotation
    else:
        return current_location, current_rotation
