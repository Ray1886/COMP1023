...
from COMP1023.assignments.programming_assignment02.utils import overlap
from COMP1023.assignments.programming_assignment02.assets import open_and_resize_photoimage, RESIZE_SCALE
...
class Map:
    ...
    def map_diff(self):
        ...
        if self.prev_map is None:
            difference = np.full((self.rows, self.cols), True)
        else:
            difference = (self.map != self.prev_map)
    ...
    def collides(
        ...
        ro = ox1, oy1, ox2, oy2
        collided_tile = {}
        if ox1 <= 0 or ox2 >= self.cols or oy1 <= 0 or oy2 >= self.rows:
            return None
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if tile == Tile.ROCK:
                    rtile = j, i, j + 1, i + 1
                    if overlap(ro, rtile):
                        collided_tile[(j,i)] = Tile.ROCK                  
                if tile == Tile.BOMB:
                    rtile = j, i, j + 1, i + 1
                    if overlap(ro, rtile):
                        collided_tile[(j,i)] = Tile.BOMB
    ...
    def collides_with_tank(
        ...
        oy2 = y + height/2
        ro = ox1, oy1, ox2, oy2
        for tank_id, (x, y) in self.tank_position_map.items():
            tx1 = x - 1/2
            ty1 = y - 1/2
            tx2 = x + 1/2
            ty2 = y + 1/2
    ...
    def trigger_bomb(
        ...
        for i in range(row -1, row +2):
            for j in range(col - 1, col + 2):
                ...
                if i == row and j == col:
                    continue
                if self.map[i][j] == Tile.BOMB and (j,i) not in to_trigger:
                    to_trigger.append((j, i))
                if self.map[i][j] != Tile.BOMB:
                    self.map[i][j] = Tile.EMPTY
                    self.draw_on(canvas)
        for (j, i) in to_trigger:
            if  0 <= i < self.rows and 0 <= j < self.cols and self.map[i][j] == Tile.BOMB:
