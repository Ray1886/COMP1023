...
def task_1_add_course(schedule, code, slots_to_add):
    ...
    for i in range(len(slots_to_add)):
        if updated_schedule[slots_to_add[i][0]][slots_to_add[i][1]] != None:
    ...
    if all_slots_free:
        ...
        for i in range(len(slots_to_add)):
            updated_schedule[slots_to_add[i][0]][slots_to_add[i][1]] = code
...
def task_3_view_slice(schedule, start_day, end_day):
    ...
    schedule_slice = schedule[start_day:end_day+1]
    for i in range(len(schedule_slice)):
        day_name = DAYS[start_day + i]
        display_row = ['---'if x == None else x for x in schedule_slice[i]]
        result_text += f"{day_name}: {display_row}\n"
