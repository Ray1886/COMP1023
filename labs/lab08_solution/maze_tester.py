...
import COMP1023.labs.lab8_solution.maze_core as maze_core
import COMP1023.labs.lab8_solution.maze_solver as maze_solver
...
class MazeTesterGUI(tk.Tk):
    ...
    def _render_maze(self):
        ...
        for y in range(h):
            for x in range(w):
                ...
                # visited overlay
                if (x, y) in self.visited and (x, y) not in [tuple(self.start), tuple(self.end)]:
        ...
        steps = len([p for p in self.visited if p != tuple(self.start)])
    ...
    def _animate_step(self):
        ...
        for pair in self.teleport_info:
            if final_pos in pair[1:]:
                ...
                # store highlights as tuples for set membership and rendering
                self.tp_from_used.add(tuple(src))
                self.tp_to_used.add(tuple(dst))
                final_pos = dst
                break

        # Revisit check
        if tuple(final_pos) in self.visited:
            self.is_animating = False
            self._render_maze()
        ...
        self.visited.add(tuple(final_pos))
    ...
    def _finalize_animation(self, completed: bool, reason: str | None):
        ...
        reached_end = (tuple(self.player_pos) == tuple(self.end))
    ...
    def _reset_visualization(self, render: bool = True):
        ...
        self.visited = set([tuple(self.start)]) if self.start else set()
        self.path_index = 0
        self.executed_steps = 0
        self.is_animating = False
        self.current_score = 0
